      ...
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-gutter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-24">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="font-inter text-sm text-white/60 leading-relaxed max-w-sm">
              Qatar's leading premium photo booth rental and event activations company. Elevating weddings, corporate events, and VIP galas.
            </p>
          </div>

          {/* Navigation */}
          <div className="flex flex-col gap-6">
            <h4 className="font-inter font-bold text-xs uppercase tracking-widest text-white/40">Navigation</h4>
            <nav className="flex flex-col gap-4">
              {['Home', 'Services', 'Portfolio', 'Contact'].map((item) => (
                <Link key={item} href={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className="font-inter text-sm font-medium text-white/80 hover:text-primary transition-colors flex items-center gap-1 group w-max">
                  {item}
          ...
          <div className="flex flex-col gap-6 lg:col-span-2">
            <h4 className="font-inter font-bold text-xs uppercase tracking-widest text-white/40">Contact Us</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div>
                <p className="font-inter text-xs text-white/50 uppercase tracking-widest mb-2">Studio Address</p>
                <p className="font-inter text-sm text-white/90 leading-relaxed">
                  Building 17, Ishbiliya Street,<br />Al Hilal, Doha, Qatar
                </p>
              </div>
              <div>
                <p className="font-inter text-xs text-white/50 uppercase tracking-widest mb-2">Get in touch</p>
                <a href="tel:+97471567348" className="block font-inter text-sm text-white/90 hover:text-primary transition-colors mb-2">+974 7156 7348</a>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="font-inter text-xs text-white/40">
            © {new Date().getFullYear()} Mirror Mirror Photowall Qatar. All rights reserved.
          </p>
          <div className="flex items-center gap-4">
            <Link href="#" className="font-inter text-xs text-white/40 hover:text-white transition-colors">Privacy Policy</Link>
            <Link href="#" className="font-inter text-xs text-white/40 hover:text-white transition-colors">Terms of Service</Link>
